...
def build_image(path_from,path_to) :
        ...
        for row in picture.iter_lvl(0) :
                ...
                for serie in recombined :
                        ...
                        new_picture.write(turninto(0,"WORD"))
                        ...
                        while i<len(serie) :
                                ...
                                if serie[i] - model not in range(-7,8):#the params of the range are to be re-worked
                                        ...
                                        new_picture.write(turninto(i,"WORD"))
...
def read_image(path_from,path_to=None):
        ...
        for y in range(height):
                ...
                row = list()
                while len(row) < 3 :
                        new = {readnext("WORD",picture) : (readnext("WORD",picture),readnext("WORD",picture))}
                        #i.e. (model_from,(,b,a))
                        j = list(new.keys())[0]
                        if j <= i :
                                row.append(channel)
...
WORD model for xx px (not yet inplemented)
WORD b
WORD a
______________________
