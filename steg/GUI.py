...
from steg import  BMP, hide_n, show_n
...
def put_photo(*args,**kwargs):
        ...
        photo = tk.filedialog.askopenfilename()#mode="rb") if tk.filedialog.askopenfile()
        if photo :
                photo = Image.open(photo)
                BMP_PICTURE = BMP(photo)
...
def update_photo(deepness):#this function will hide the message into the picture, on <deepness> bits per byte
        ...
        hide_n(BMP_PICTURE,msg,int(deepness))
        PICTURE = ImageTk.PhotoImage(data=BMP_PICTURE.to_bytes())
        canvas.create_image(0,0,image=PICTURE,anchor="nw")
...
def export_picture(*args,**kwargs):
        ...
        f = tk.filedialog.asksaveasfile(mode="wb")
        f.write(BMP_PICTURE.to_bytes())
